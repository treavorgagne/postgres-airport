...
# aircraft consistency constraint by scheduling a flight which originates at an 
# airport different from the aircraft's current location at the time.
echo 'Running invalid flight create operation (should fail)'
python3 ../solution/manage_flights.py flights2.txt

#These reports should be identical to the "round 1" reports (since the 
#operation on line 22 failed and was rolled back)
...
# This command attempts to modify an existing flight (which is in compliance
# with the consistency constraint) and change it scheduled time such that the 
# constraint is violated. It should fail.
echo 'Running invalid flight update operation (should fail)'
python3 ../solution/manage_flights.py flights3.txt

#These reports should be identical to the "round 1" reports (since the 
#operation on line 34 failed and was rolled back)
...
# This command attempts to delete a flight such that the resulting schedule
# is no longer consistent.
echo 'Running invalid flight delete operation (should fail)'
python3 ../solution/manage_flights.py flights4.txt

#These reports should be identical to the "round 1" reports (since the 
#operation on line 44 failed and was rolled back)
python3 ../solution/report_all_flights.py > output_report_all_flights_round4.txt
python3 ../solution/report_aircraft.py > output_report_aircraft_round4.txt